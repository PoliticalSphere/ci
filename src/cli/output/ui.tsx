...
import type { LinterStatus } from '../execution/executor.ts';
...
function supportsOsc8(): boolean {
  const env = process.env as { TERM?: string | undefined; TERM_PROGRAM?: string | undefined };
  const term = env.TERM;
  ...
  // Fallback to basic TERM check. Treat the literal string 'undefined' (which can appear
  // when tests assign `process.env[KEY] = undefined`) as unset so we behave like when
  // the environment variable is absent.
  return typeof term === 'string' && term !== '' && term !== 'dumb' && term !== 'undefined';
}
...
const supportsColor = (): boolean => {
  // Be tolerant of tests setting env to `undefined` (which can leave a string 'undefined' in
  // some environments) — treat absent or 'undefined' as unset.
  const noColor = (process.env as { NO_COLOR?: string | undefined }).NO_COLOR;
  if (noColor !== undefined && noColor !== 'undefined') {
    return false;
  }

  const term = (process.env as { TERM?: string | undefined }).TERM;
  if (term === 'dumb') {
    return false;
  }
  return true;
...
// Utilities used from tests are exported at the bottom of the file to avoid
// referencing components before they are defined.
...
> {
  ...
  public override render(): React.ReactNode {
    if (this.state.hasError) {
      return (
        <Box flexDirection="column" padding={1}>
          <Text bold>Dashboard error</Text>
          <Text>UI failed to render. Check logs for details.</Text>
...
// Expose testing-only helpers and components
