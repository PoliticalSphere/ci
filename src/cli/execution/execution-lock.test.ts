...
describe('Political Sphere — Execution Lock', () => {
  ...
  it('treats lock as stale when age exceeds threshold (~60s)', async () => {
    const lockPath = path.join(tempDir, 'ps-parallel-lint.lock');
    const now = Date.now();
    // Create lock older than MAX_LOCK_AGE_MS (60s)
    const createdAt = now - 60 * 1000 - 1;
    await writeFile(lockPath, JSON.stringify({ pid: process.pid, createdAt }), 'utf8');
    ...
    // Should not enter waiting state; lock removed as stale by age
    expect(onWaitStart).not.toHaveBeenCalled();
    await lock.release();
  });
