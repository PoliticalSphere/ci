...
export class TelemetryCollector {
   ...
   *
   * @param enabled - Whether to collect metrics (defaults to true).
   */
   ...
   *
   * @param linterId - Identifier of the running linter.
   * @param ctx - Trace context used for correlation.
   * @returns Tracker used later to record metrics for this run.
   */
  startExecution(
    linterId: string,
    ctx: TraceContext,
   ...
   *
   * @param execution - Tracker returned by `startExecution`.
   * @param outputBytes - Number of bytes written to logs.
   * @param success - Whether the execution succeeded.
   * @param errorMessage - Optional error message when failing.
   */
  recordExecution(
  ...
  /**
   * Get metrics for a specific trace ID.
   */
  ...
  /**
   * Calculate aggregate statistics from tracked metrics.
   *
   * @param filter - Optional predicate to include only a subset of metrics.
   * @returns Aggregated stats (total/executions/durations/bytes).
   */
  ...
  /**
   * Export metrics for ingestion or inspection.
   */
  export(): object {
    return {
      version: '1.0',
  ...
  /**
   * Export metrics filtered to a specific linter id.
   */
 ...
 *
 * @param enabled - When false, returns a no-op collector.
 */
