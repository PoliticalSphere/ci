...
// Helper to create a logger with a fixed clock (reduces repeated setup calls)
async function createLoggerWithFixedTime(options: {
  linterId: string;
  verifyMode: boolean;
  structured?: boolean;
  maxBytes?: number;
  ...
  return createLogger({ logDir: tempDir, ...options });
}
...
describe('Political Sphere — Logger', () => {
  ...
  it('normalizes output in standard mode and preserves blank lines', async () => {
    const logger = await createLoggerWithFixedTime({
      linterId: 'eslint',
      verifyMode: false,
    });
  ...
  it('creates the log directory when appending to a missing path', async () => {
    const nestedDir = path.join(tempDir, 'nested-logs');
    await appendToLog(nestedDir, 'eslint', 'created');

    const out = await readFile(path.join(nestedDir, 'eslint.log'), 'utf8');
    expect(out).toBe('created\n');
  });

  it('creates the log directory when creating a logger', async () => {
    const nestedDir = path.join(tempDir, 'nested-logger');
    const logger = await createLogger({
      logDir: nestedDir,
      linterId: 'biome',
      verifyMode: true,
    ...
    await logger(Readable.from(['hello\n']));
    const out = await readFile(path.join(nestedDir, 'biome.log'), 'utf8');
    expect(out).toBe('hello\n');
  });
