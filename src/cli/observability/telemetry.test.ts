...
describe('Political Sphere — Telemetry', () => {
  describe('TelemetryCollector', () => {
    ...
    describe('exportByLinter', () => {
      it('exports metrics for specific linter', () => {
        ...
        expect((exported.stats as { totalExecutions?: number }).totalExecutions).toBe(1);
      });
    });
  });
  ...
  describe('Global telemetry', () => {
    ...
    describe('getGlobalTelemetry', () => {
      ...
      it('returns a disabled collector when initialized with false', async () => {
        vi.resetModules();
        const mod = await import('./telemetry.ts');
        const telemetry = mod.getGlobalTelemetry(false);
        const ctx = recordOneMetric(telemetry, 'eslint', 100, true);
        expect(telemetry.getMetric(ctx.traceId)).toBeUndefined();
      });
    });
