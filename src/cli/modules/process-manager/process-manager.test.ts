 ...
 *
 * Validates `runProcess`, timeout/killing behavior, and the transient error utility.
 */
...
describe('Process Manager Module', () => {
  ...
  describe('killProcessTree', () => {
    it('does not throw when pid is undefined', () => {
      const proc = { pid: undefined, kill: vi.fn() } as unknown as ReturnType<typeof spawn>;
      expect(() => processManager.killProcessTree(proc)).not.toThrow();
      expect(proc.kill).not.toHaveBeenCalled();
    });

    it('sends SIGKILL when graceful is false', () => {
      const proc = { pid: 1234, kill: vi.fn() } as unknown as ReturnType<typeof spawn>;
      const killSpy = vi.spyOn(process, 'kill').mockImplementation(() => true as never);

      try {
        processManager.killProcessTree(proc, false);
  ...
  describe('killProcessTree (edge cases)', () => {
    beforeEach(() => {
      vi.clearAllMocks();
    });
