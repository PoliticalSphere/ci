/**
 * Tests for the binary checker module, covering resolution, probes, and version utilities.
 */
...
import {
  checkBinaryExists,
  runVersionProbe,
  verifyAllLinterBinaries,
  verifyLinterVersion,
} from './binary-checker.ts';

vi.mock('node:child_process');
vi.mock('node:fs/promises');

/**
 * Build a lightweight fake process object that mimics the minimal shape of
 * a child_process `Spawn` return value used by the production probe logic.
 */
const buildProc = () =>
...
/** Emit a chunk of data on the fake process stdout and close it. */
const emitProcData = (proc: ReturnType<typeof buildProc>, data: string) => {
  proc.stdout.emit('data', Buffer.from(data));
  proc.emit('close', 0);
};

/** Create a spawn mock that asynchronously emits `output` then exits. */
...
/** Create a spawn mock that may emit output (or not) then exits. */
...
describe('Binary Checker Module', () => {
  ...
  describe('checkBinaryExists', () => {
    ...
    it('accepts absolute or relative binary paths', async () => {
      ...
      const fakeStat: { isFile: () => boolean } = { isFile: () => true };
      vi.mocked(stat).mockResolvedValueOnce(fakeStat as never);
      vi.mocked(realpath).mockResolvedValueOnce('/resolved/tools/my-binary');
    ...
    it('includes absolute PATH entries when searching', async () => {
      ...
      const expectedStat: { isFile: () => boolean } = { isFile: () => true };
      vi.mocked(stat).mockResolvedValue(expectedStat as never);
      vi.mocked(realpath).mockResolvedValue(expectedPath);

      await withEnv({ PATH: '/custom/bin:relative/bin' }, async () => {
        const result = await checkBinaryExists('tool');
        expect(result).toBe(true);
    ...
    it('returns true when binary exists', async () => {
      vi.mocked(access).mockResolvedValueOnce(undefined);
      const lsStat: { isFile: () => boolean } = { isFile: () => true };
      vi.mocked(stat).mockResolvedValueOnce(lsStat as never);
      vi.mocked(realpath).mockResolvedValueOnce('/usr/bin/ls');
    ...
    it('returns false when candidate is not a file', async () => {
      vi.mocked(access).mockResolvedValueOnce(undefined);
      const notFileStat: { isFile: () => boolean } = { isFile: () => false };
      vi.mocked(stat).mockResolvedValueOnce(notFileStat as never);

      const result = await checkBinaryExists('not-file');
  ...
  describe('runVersionProbe', () => {
    ...
    it('rejects when output exceeds max bytes', async () => {
      const proc = buildProc();
      const kill = vi.fn();
      proc.kill = kill;
      vi.mocked(spawn).mockReturnValue(proc);
      ...
      const promise = runVersionProbe('eslint', ['--version']);
      queueMicrotask(() => {
        proc.stdout.emit('data', Buffer.from('a'.repeat(8 * 1024 + 1)));
      });

      await expect(promise).rejects.toThrow('Version probe output exceeded');
  ...
  describe('verifyLinterVersion', () => {
    it('returns null when expectedVersion is not set', async () => {
      const result = await verifyLinterVersion({ id: 'demo' });
      expect(result).toBeNull();
    });

    it('returns null when expectedVersion is 0.0.0 sentinel', async () => {
    ...
    it('verifyAllLinterBinaries returns missing entries', async () => {
      // Simulate environment where only 'eslint' is present
      vi.mocked(access).mockImplementation(async (p: string, _mode?: number) => {
        if (p.includes('eslint')) {
          return undefined;
        }
        throw new Error('missing');
      });
      const eslintStat: { isFile: () => boolean } = { isFile: () => true };
      vi.mocked(stat).mockImplementation(async (p: string) => {
        if (p.includes('eslint')) {
          return eslintStat as never;
        }
        throw new Error('missing');
      });
      ...
      const result = await verifyAllLinterBinaries([
        { id: 'eslint', binary: 'eslint' },
        { id: 'gitleaks', binary: 'gitleaks' },
      ]);

      expect(result).toEqual(['gitleaks']);
    });
    ...
    it('returns null when version matches', async () => {
      ...
      const eslintStat: { isFile: () => boolean } = { isFile: () => true };
      vi.mocked(stat).mockResolvedValue(eslintStat as never);
      vi.mocked(realpath).mockResolvedValue('/usr/bin/eslint');
    ...
    it('returns null when expectedVersion is the sentinel 0.0.0', async () => {
      const result = await verifyLinterVersion({
        id: 'eslint',
        expectedVersion: '0.0.0',
        versionProbe: { binary: 'eslint', args: ['--version'] },
