...
// Local, minimal type for test fixtures — mirrors production `LinterResult` shape
...
/**
 * Helper shape for deferred lock acquisition handlers used in tests.
 */
interface LockDeferred {
  promise: Promise<{ lockPath: string; release: () => Promise<void> }>;
  resolve: (value: { lockPath: string; release: () => Promise<void> }) => void;
  reject: (err: unknown) => void;
}

/**
 * Create a generic lock acquisition handler from a deferred promise.
 */
export function createLockAcquisitionHandler(
  lockDeferred: LockDeferred,
): (options?: {
  onWaitStart?: () => void;
  onWaitEnd?: () => void;
}) => Promise<{ lockPath: string; release: () => Promise<void> }> {
  return (options?: { onWaitStart?: () => void; onWaitEnd?: () => void }) => {
    options?.onWaitStart?.();
