...
. "${script_dir}/../../core/path-validation.sh"
...
# safe_relpath_or_die is now provided by core/path-validation.sh
...
section() {
  ...
  if [[ -f "${format_sh}" ]]; then
    ...
    . "${format_sh}"
    ps_print_section "${id}" "${title}" "${desc}"
  else
    printf '\n== %s ==\n' "${title}"
    [[ -n "${desc}" ]] && printf '%s\n' "${desc}"
  fi
...
node "${repo_root}/tools/scripts/runners/security/license-check.js" \
  --policy "${policy_path}" \
  --lock "${lock_path}" \
  --report "${report_path}" \
