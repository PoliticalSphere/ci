...
req="$(require_bool "inputs.require_full_history" "${PS_REQUIRE_FULL_HISTORY_INPUT:-false}")"
if [[ "${req}" == "1" && "${depth}" != "0" ]]; then
  printf 'ERROR: full history required but fetch_depth=%s (expected 0)\n' "${depth}" >&2
  exit 1
fi

pc="$(require_bool "inputs.persist_credentials" "${PS_PERSIST_CREDENTIALS_INPUT:-false}")"
