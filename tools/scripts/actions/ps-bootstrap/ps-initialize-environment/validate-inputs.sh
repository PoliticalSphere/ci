...
# -----------------------------------------------------------------------------
PS_VALIDATION_START_TIME="$(date +%s)"
...
# Performance metrics
# -----------------------------------------------------------------------------
PS_VALIDATION_END_TIME="$(date +%s)"
PS_VALIDATION_DURATION="$((PS_VALIDATION_END_TIME - PS_VALIDATION_START_TIME))"
emit_env "PS_INIT_VALIDATION_DURATION_SEC" "${PS_VALIDATION_DURATION}"

# -----------------------------------------------------------------------------
# Summary
# -----------------------------------------------------------------------------
printf 'PS.INIT: validated (repo depth=%s, platform=%s, tools=%s, validation=%ss)\n' \
  "${depth}" \
  "$([[ "${skip_platform}" == "1" ]] && echo "off" || echo "on")" \
  "$([[ "${install_tools}" == "1" ]] && echo "${bundle}" || echo "off")" \
  "${PS_VALIDATION_DURATION}"
  "${depth}" \
  "$([[ "${skip_platform}" == "1" ]] && echo "off" || echo "on")" \
  "$([[ "${install_tools}" == "1" ]] && echo "${bundle}" || echo "off")"
exit 0
