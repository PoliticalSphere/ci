...
function runAssemble(inputsEnv) {
  try {
    const env = Object.assign({}, process.env, inputsEnv);
...
try {
  ...
  {
    const env = { PS_TOOLS_BUNDLE_INPUT: 'lint', PS_TOOLS_EXTRA_INPUT: '', PS_PLATFORM_ROOT: repoRoot };
    const out = execFileSync('bash', ['-lc', `bash -lc "${repoRoot}/.github/actions/ps-tools/action.yml" 2>/dev/null || true`], { encoding: 'utf8', env });
    // Instead of trying to execute the composite action, assert the bundle logic by reading file
    const actionYml = readFileSync(`${repoRoot}/.github/actions/ps-tools/action.yml`, 'utf8');
    assert(actionYml.includes("bundle: \"lint\"") || actionYml.includes("bundle: \"security\""));
  }
