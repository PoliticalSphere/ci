...
assert.equal(
  normalizeActionToRepo('owner/repo@abcdef1234567890abcdef1234567890abcdef12'),
  'owner/repo',
);
...
assert.deepEqual(mapStatusToInfo(401), {
  reason: 'authentication failed',
  error: 'unauthorized',
});
assert.deepEqual(mapStatusToInfo(403), {
  reason: 'permission/rate limited',
  error: 'forbidden_or_rate_limited',
});
assert.deepEqual(mapStatusToInfo(429), {
  reason: 'rate limited',
  error: 'rate_limited',
});
assert.deepEqual(mapStatusToInfo(500), {
