...
const r2 = runShell(String.raw`source tools/scripts/branding/validate-inputs.sh; tools_raw='badTool!'; tools_trimmed="$(printf '%s' "$tools_raw" | sed 's/^\s*//; s/\s*$//')"; while IFS= read -r t; do t_trim="$(printf '%s' "$t" | sed 's/^\s*//; s/\s*$//')"; if [[ -z "$t_trim" ]]; then continue; fi; if ! printf '%s' "$t_trim" | grep -Eq '^[a-z0-9-]+$'; then v_error "invalid tool id in inputs.tools: $t_trim (allowed: lowercase letters, digits, hyphen)"; exit 1; fi; done <<< "$tools_trimmed"; echo OK`)
...
const r3 = runShell("source tools/scripts/branding/validate-inputs.sh; PS_BUNDLE_INPUT=none; extra_trimmed=''; if [[ \"$PS_BUNDLE_INPUT\" == \"none\" && -z \"$extra_trimmed\" ]]; then v_error \"no tools selected (bundle=none and extra_tools empty). If you intended to provide explicit tools, use the 'tools' input.\"; exit 1; fi; echo OK")
