...
let r = runShell("source tools/scripts/branding/validate-inputs.sh; tools_raw='actionlint,rm -rf /'; require_regex 'inputs.tools' \"$tools_raw\" '^[a-z0-9,-]+$' 'hint' || exit 0; echo OK");
if (r.status === 0 && r.stdout.includes('OK')) {
  fail('require_regex allowed malicious tools_raw');
}

// 2) require_nonempty rejects empty
r = runShell("source tools/scripts/branding/validate-inputs.sh; require_nonempty 'inputs.script' '' || exit 0; echo OK");
if (r.status === 0 && r.stdout.includes('OK')) {
  fail('require_nonempty accepted empty value');
}

// 3) require_number rejects non-number
r = runShell("source tools/scripts/branding/validate-inputs.sh; require_number 'inputs.node_version' 'foo' || exit 0; echo OK");
if (r.status === 0 && r.stdout.includes('OK')) {
